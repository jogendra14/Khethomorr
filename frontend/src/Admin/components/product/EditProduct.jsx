...
const EditProduct = () => {
  ...
  // ✅ UPDATED: Safely extract category & subCategory IDs if populated as objects from backend
  useEffect(() => {
    if (productData) {
      const data = productData;

      // Extract string ID if category or subCategory is populated as an object
      const categoryId = typeof data.category === "object" && data.category !== null 
        ? (data.category._id || data.category.id || "") 
        : (data.category || "");

      const subCategoryId = typeof data.subCategory === "object" && data.subCategory !== null 
        ? (data.subCategory._id || data.subCategory.id || "") 
        : (data.subCategory || "");
  ...
  const { data: subCategoriesData } = useQuery({
    ...
    enabled: !!formData.category && typeof formData.category === "string",
  });
  
  // ✅ UPDATED: Robust SubCategories resolver (Nested + API Multi-Format Support)
  useEffect(() => {
    ...
    // Method 1: Check if subcategories exist inside selected category object itself
